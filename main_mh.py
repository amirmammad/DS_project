...
#class for static methods and fields
class Static:
    #tokenizing
    @staticmethod
    def tokenize(text):
    ...
    #giving input and run the program
    @staticmethod
    def InputGir(Query, Doc_List):
        query = Query
        Static.dim = set(Static.tokenize(query))
        for doc_num in Doc_List:
            Static.docs_text[doc_num] = open(os.getcwd() + "\\data\\document_" + str(doc_num) + ".txt", "r", encoding='utf-8').read()
        system = Program(query, Doc_List)

    query = str()
    dim = set()
    docs_text = dict()
    most_related_doc_number = int()
...
class Document:
    def __init__(self, doc_text):
        ...
        self.doc_lines_tf = self.line_1_tf_calculator(Static.tokenize(doc_text))
    ...
    def line_1_idf_calculator(self, doc_text):
        ...
        for term in Static.dim:
            for line in doc_lines:
                if term in Static.tokenize(line):
                    term_counter[term] += 1
        idf = dict()
        for term in Static.dim:
...
class Query_1:
    ...
    def query_1_idf_calculator(self, doc_list):
        ...
        for term in Static.dim:
            for doc_num in doc_list:
                tokenized_doc_text = Static.tokenize(Static.docs_text[doc_num])
                if term in tokenized_doc_text:
                    doc_counter[term] += 1
        idf = dict()
        for term in Static.dim:
            if doc_counter[term] == 0:
                idf[term] = log((len(doc_list)) / 0.001)
            else:
    ...
    def query_1_tf_calculator(self, doc_list):
        ...
        for term in Static.dim:
            term_counter[term] = 0
        for doc_num in doc_list:
            doc_words = Static.tokenize(Static.docs_text[doc_num])
            for term in doc_words:
                if term in Static.dim:
                    term_counter[term] += 1
        query_words = Static.tokenize(Static.query)
...
class Query_2:
    ...
    def query_2_tf_calculator(self, doc_text):
        ...
        for term in Static.dim:
            term_counter[term] = 0
        doc_words = Static.tokenize(doc_text)
        for term in doc_words:
            if term in Static.dim:
                term_counter[term] += 1
        query_words = Static.tokenize(Static.query)
...
class Line_2:
    def __init__(self, idf, line_text):
        ...
        self.idf = idf
        self.tf = self.line_2_tf_calculator(Static.tokenize(line_text))
...
class Program:
    def __init__(self, query, doc_list):
        ...
        self.doc_dict = dict()
        for doc_num in doc_list:
            self.doc_dict[doc_num] = Document(Static.docs_text[doc_num])
        Static.most_related_doc_number = self.max_comparator(self.doc_dict, type="doc")
        self.most_related_doc_text = self.doc_dict[Static.most_related_doc_number].doc_text 
        print(Static.most_related_doc_number)
    ...
    def max_comparator(self, search_domain, type):
        ...
        for instance in search_domain:
            if type == "doc":
                similarity_score_dict[instance] = 0.4 * (self.cosine_similarity(self.query_1.vector, search_domain[instance].doc_vector)) + 0.6 * (SequenceMatcher(None, Static.query, search_domain[instance].doc_text).ratio())
            else:
                similarity_score_dict[search_domain.index(instance)] = 0.6 * (self.cosine_similarity(self.query_2.vector, instance.vector)) + 0.4 * (SequenceMatcher(None, Static.query, instance.line_text).ratio())
